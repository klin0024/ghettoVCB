...
tohtml(){
	...
	date=$(date +%F)
    time=$(date +%F" "%H:%M:%S)
    #htmlfile=/vmfs/volumes/backup/ghettoVCB/web/servers/${BACKUPNAME}/${BACKUPNAME}-${date}.html
	...
	if [[ "${vm_status}" -eq 1 ]] ; then
    	html="<tr><td>${time}</td><td>${BACKUPNAME}</td><td>${VM_NAME}</td><td>${vm_size}</td><td>SUCCESSED</td></tr>"

    else
    	html="<tr class='danger'><td>${time}</td><td>${BACKUPNAME}</td><td>${VM_NAME}</td><td>${vm_size}</td><td>FAILED</td></tr>"
    fi
...
#read user input
while getopts ":af:c:g:w:m:l:L:d:e:" ARGS; do
    case $ARGS in
        w)
            WORKDIR="${OPTARG}"
            ;;
